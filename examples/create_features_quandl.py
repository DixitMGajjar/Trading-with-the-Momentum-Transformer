...
from settings.default import (
    ...
    FEATURES_QUANDL_FILE_PATH,YF_TICKERS
)
...
def main(
    ...
    if lookback_window_length:
        ...
        if extra_lbw:
            for extra in extra_lbw:
                extra_file_path = output_file_path.replace(  # ADD THIS LINE
                    f"quandl_cpd_{lookback_window_length}lbw.csv",
                    f"quandl_cpd_{extra}lbw.csv",
                )
                if not os.path.exists(extra_file_path):  # ADD THIS LINE
                    print(f"Warning: {extra_file_path} does not exist, skipping...")  # ADD THIS LINE
                    continue  # ADD THIS LINE
                
                extra_data = pd.read_csv(
                    extra_file_path,  # CHANGE: use the variable instead of inline replace
                    index_col=0,
                    parse_dates=True,
                ).reset_index()[
                    ["date", "ticker", f"cp_rl_{extra}", f"cp_score_{extra}"]
