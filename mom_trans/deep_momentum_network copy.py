...
# Example (depending on version)
# from keras_tuner import callbacks as kt_callbacks

# callbacks.append(kt_callbacks.TunerCallback(self, trial))
...
class SharpeValidationLoss(keras.callbacks.Callback):
    def __init__(
        ...
        self.best_sharpe = -np.inf  # Changed from np.inf to -np.inf since we're maximizing Sharpe
        self.weights_save_location = weights_save_location
        
        # ADD THIS: Create directory if it doesn't exist
    ...
    def set_weights_save_loc(self, weights_save_location):
        self.weights_save_location = weights_save_location
        # ADD THIS: Create directory for the new location too
        directory = os.path.dirname(weights_save_location)
        if directory and not os.path.exists(directory):
            os.makedirs(directory, exist_ok=True)
    ...
    def on_train_begin(self, logs=None):
        self.patience_counter = 0
        self.stopped_epoch = 0
        self.best_sharpe = -np.inf  # Changed from np.inf to -np.inf
    ...
    # def on_epoch_end(self, epoch, logs=None):
        ...
        # # FIXED: Changed comparison since we're maximizing Sharpe (higher is better)
        # if sharpe > self.best_sharpe + self.min_delta:
        #     self.best_sharpe = sharpe
        #     self.patience_counter = 0  # reset the count
        #     # self.best_weights = self.model.get_weights()
        ...
        #         self.stopped_epoch = epoch
        #         self.model.stop_training = True
        #         # FIXED: Only load weights if the file exists
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # ADD: Handle NaN values
        if np.isnan(sharpe) or np.isinf(sharpe):
            sharpe = -10.0  # Very poor Sharpe for NaN/inf cases
...
class TunerValidationLoss(kt.tuners.RandomSearch):
    ...
    def __init__(
        ...
        tune_new_entries=True,
        allow_new_entries=True,
        max_consecutive_failed_trials=3,  # ADD THIS LINE
        **kwargs,
    ):
        self.hp_minibatch_size = hp_minibatch_size
        kwargs['max_consecutive_failed_trials'] = max_consecutive_failed_trials  # ADD THIS LINE
        super().__init__(
            hypermodel,
            objective,
            max_trials,
...
class TunerDiversifiedSharpe(kt.tuners.RandomSearch):
    ...
    #     # Convert trial_id to integer first, then format with zero-padding
    #     trial_id_int = int(trial_id)
    #     return os.path.join(self.project_name, f"trial_{trial_id_int:02d}.h5")
    def _get_checkpoint_fname(self, trial_id):
        """Return a path to save checkpoint weights for a trial."""
        os.makedirs(self.project_name, exist_ok=True)
        # Convert trial_id to integer first, then format with zero-padding
        # If trial_id is a string like "00", int("00") becomes 0.
        try:
            trial_id_int = int(trial_id)
            return os.path.join(self.project_name, f"trial_{trial_id_int:02d}.weights.h5")
        except ValueError:
            # Fallback if trial_id is not an integer string
            return os.path.join(self.project_name, f"trial_{trial_id}.weights.h5")
        # return os.path.join(self.project_name, f"trial_{trial_id}.h5")
    ...
    def run_trial(self, trial, *args, **kwargs):
        ...
        #         fname = self._get_checkpoint_fname(trial.trial_id, self._reported_step)
        ...
        for callback in original_callbacks:
            if isinstance(callback, SharpeValidationLoss):
                # FIXED: Removed self._reported_step argument
                fname = self._get_checkpoint_fname(trial.trial_id)
                
                # Redundant safety check (filename already has extension from _get_checkpoint_fname)
                if not fname.endswith(".weights.h5"):
                    fname += ".weights.h5"
                callback.set_weights_save_loc(fname)
        metrics = collections.defaultdict(list)
        for execution in range(self.executions_per_trial):
            ...
            # Pass the actual data arguments to _build_and_fit_model
            history = self._build_and_fit_model(trial, *args, **copied_fit_kwargs)
            for metric, epoch_values in history.history.items():
                if self.oracle.objective.direction == "min":
                    best_value = np.min(epoch_values)
...
class DeepMomentumNetworkModel(ABC):
    ...
    # def hyperparameter_search(self, train_data, valid_data):
        ...
        # Use the dictionary directly without unpacking
        # data = train_data
        ...
        #     # )
        #     self.tuner.search(
        #         x=data["inputs"],  # Pass the inputs directly
        #         y=data["outputs"],  # Pass the outputs directly as labels
        #         sample_weight=data.get("active_entries", None),  # Use active_entries if available
        #         epochs=self.num_epochs,
        #         callbacks=callbacks,
        #         shuffle=True,
        #         use_multiprocessing=True,
    ...
    # def hyperparameter_search(self, train_data, valid_data):
    #     # Use the dictionaries directly without unpacking
    #     data = train_data
    #     validation_data = valid_data

    #     if self.evaluate_diversified_val_sharpe:
    #         # For diversified Sharpe, we need time information from validation data
    #         val_data, val_labels, val_flags, _, val_time = ModelFeatures._unpack(valid_data)
    #         val_time_indices, num_val_time = self._index_times(val_time)
    ...
    def hyperparameter_search(self, train_data, valid_data):
        try:
            ...
            # Return default values or re-raise
            raise
    def load_model(
        self,
        hyperparameters,
    ...
    def fit(
        ...
        if self.evaluate_diversified_val_sharpe:
            ...
            # Ensure consistency if temp_folder is just a path prefix
            if not temp_folder.endswith(".weights.h5") and not os.path.exists(temp_folder):
