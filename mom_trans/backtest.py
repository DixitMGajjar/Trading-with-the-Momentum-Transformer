...
def run_single_window(
    ...
    print("Train start:", train_interval[0])
    print("Test start:", train_interval[1])
    print("Test end:", train_interval[2])
    print("Raw data shape:", raw_data.shape)
    print("Columns:", raw_data.columns)
    print("First 5 rows:\n", raw_data.head())

    if model_features.train:
        # print("Train shapes:", model_features.train[0].shape, model_features.train[1].shape)
        print("Train keys:", model_features.train.keys())
        for k, v in model_features.train.items():
            if isinstance(v, np.ndarray) or isinstance(v, pd.DataFrame):
                print(f"Train {k} shape: {v.shape}")

    else:
        print("model_features.train is empty")

    if model_features.valid:
        # print("Valid shapes:", model_features.valid[0].shape, model_features.valid[1].shape)
        print("Valid shapes:", model_features.valid["inputs"].shape, model_features.valid["outputs"].shape)

    else:
        print("model_features.valid is empty")
    ...
    # # FIXED: Added .weights.h5 extension for Keras 3
    ...
    # --- FIX: Create the directory before saving ---
    checkpoints_dir = os.path.join(best_directory, "checkpoints")
    if not os.path.exists(checkpoints_dir):
        os.makedirs(checkpoints_dir)
    # -----------------------------------------------

    best_model.save_weights(os.path.join(checkpoints_dir, "checkpoint.weights.h5"))
    
    # Ensure best_directory exists for the json file too (it should be covered by the above, but safe to check)
