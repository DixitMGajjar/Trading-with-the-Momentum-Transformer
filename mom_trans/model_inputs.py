...
class ModelFeatures:
    ...
    def set_scalers(self, df):
        ...
        # --- SAFE CLEANING FOR REAL-VALUED INPUTS BEFORE SCALING ---
# Work on a copy to avoid mutating original df
        data_df = df[real_inputs].copy()

        # Replace infinities with NaN
        data_df.replace([np.inf, -np.inf], np.nan, inplace=True)

        # Mask extremely large outliers that will not fit float64 (safety threshold)
        # You can lower this if your domain never needs numbers that big.
        _max_allowed = 1e100
        data_df = data_df.mask(data_df.abs() > _max_allowed, np.nan)

        # Fill NaNs with column median (robust to outliers); fallback to 0 if column is all NaN
        medians = data_df.median()
        data_df = data_df.fillna(medians).fillna(0.0)

        # Final sanity check
        if not np.isfinite(data_df.values).all():
            # optional: save to inspect for debugging
            data_df.to_csv("debug_nonfinite_real_inputs.csv", index=False)
            raise ValueError("Non-finite values remain in real-valued inputs after cleaning")

        # Fit real scaler
        self._real_scalers = sklearn.preprocessing.StandardScaler().fit(data_df.values)

        # --- SAFE CLEANING FOR TARGET (used for predictions) ---
        target_df = df[[target_column]].copy()
        target_df.replace([np.inf, -np.inf], np.nan, inplace=True)
        target_df = target_df.mask(target_df.abs() > _max_allowed, np.nan)
        target_df = target_df.fillna(target_df.median()).fillna(0.0)

        if not np.isfinite(target_df.values).all():
            target_df.to_csv("debug_nonfinite_target.csv", index=False)
            raise ValueError("Non-finite values remain in target column after cleaning")

        self._target_scaler = sklearn.preprocessing.StandardScaler().fit(target_df.values)
    ...
    def transform_inputs(self, df):
        ...
        # Format real inputs
        if self.transform_real_inputs:
            # Apply same cleaning as in set_scalers
            data_df = df[real_inputs].copy()
            data_df.replace([np.inf, -np.inf], np.nan, inplace=True)
            data_df = data_df.mask(data_df.abs() > 1e100, np.nan)
            medians = data_df.median()
            data_df = data_df.fillna(medians).fillna(0.0)
            
            if not np.isfinite(data_df.values).all():
                data_df.to_csv("debug_transform_inputs.csv", index=False)
                raise ValueError("Non-finite values in transform_inputs after cleaning")
            
            output[real_inputs] = self._real_scalers.transform(data_df.values)
