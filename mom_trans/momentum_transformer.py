...
# Ensure Lambda is available if referenced directly
...
def tf_stack(x, axis=0):
    if not isinstance(x, list):
        x = [x]
    # Use tf.stack instead of K.stack for better Keras 3 compatibility
...
def get_decoder_mask(self_attn_inputs):
    ...
    Args:
        self_attn_inputs: Inputs to self attention layer to determine mask shape
    """
    # Wrap the calculation in a Lambda layer so it executes within the graph
    def _compute_mask(x):
        len_s = tf.shape(x)[-2]
        bs = tf.shape(x)[:-2]
        return tf.cumsum(tf.eye(len_s, batch_shape=bs), -2)

    return keras.layers.Lambda(_compute_mask)(self_attn_inputs)
...
class TftDeepMomentumNetworkModel(DeepMomentumNetworkModel):
    ...
    def model_builder(self, hp):
        ...
        #     )
        if unknown_inputs is not None:
            # Use the Layer class instead of the backend function
            historical_inputs = keras.layers.Concatenate(axis=-1)([
                unknown_inputs,
                known_combined_layer,
            ])
        else:
            # No concatenation needed for a single tensor
        ...
        def lstm_combine_and_mask(embedding):
            ...
            # Merge the last two dimensions: embedding_dim and num_inputs
            flatten = keras.layers.Reshape((time_steps, embedding_dim * num_inputs))(embedding)
    ...
    def get_tft_embeddings(self, all_inputs):
        ...
        if self._static_input_loc:
            ...
            static_inputs = keras.layers.Lambda(lambda x: tf.stack(x, axis=1))(static_inputs)
        ...
        #     )
        if unknown_inputs + wired_embeddings:
            unknown_inputs = keras.layers.Lambda(lambda x: tf.stack(x, axis=-1))(
        ...
        # )
        known_combined_layer = keras.layers.Lambda(lambda x: tf.stack(x, axis=-1))(
